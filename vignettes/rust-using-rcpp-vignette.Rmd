...
All the examples in the documentation for `ru` are replicated in the documentation for `ru_rcpp`.  Here, we consider a subset of the examples from the [Introducing rust](rust-vignette.html) vignette to illustrate how to provide user-supplied C++ functions to `ru_rcpp` and to compare the performances of `ru` and `ru_rcpp`.  We use the **microbenchmark** package [@microbenchmark] to make the comparisons.
...
It is assumed that the user has already compiled their C++ functions and made them available to the environment of their R session, either using the `Rcpp::sourceCpp` function (e.g. `Rcpp::sourceCpp("user_fns.cpp")`) or using RStudio's Source button on the editor toolbar.
